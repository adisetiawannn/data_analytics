...
data from January 2022 untill July 2023 - just click this link to
download the data :
[link](https://divvy-tripdata.s3.amazonaws.com/index.html)

once get that data, we wiil stored that data on database, in this case
...
> do this step until you adding all cyclistic data
...
Data would be processed over 8,825,826 total records, spreadsheets
wouldn't be able to handle the sheer amount of data. In this case, we
would be using Rstudio.
...
we just have a raw data,inspect data type of each column. we can use some functions such as skim_without_chart (skimr), str (base) or glimpse (dplyr). in this case, i preferably using str () function.
...
in this case, we not removing or put some works on rows with missing values.
...
as you can see, we got some ambiguous data in trip_duration column cause it's contain trip duration < 0 sec. However, in this case, we would just create a new dataframe that does not contain trip durations of < 0 and stored as data_final.
...
I will be using ggplot in RStudio for this part of the article.Now it’s time to analyze the data and look for key information that we can perform analysis on, and afterward, plot/visualize it!
